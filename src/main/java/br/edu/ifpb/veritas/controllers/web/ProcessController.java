...
public class ProcessController {
    ...
    public ModelAndView listAll(ModelAndView model, @RequestParam(defaultValue = "1") int page,
        ...
        Page<Process> pageProcesses = processService.findAll(paging);
        NavPage navPage = NavePageBuilder.newNavPage(pageProcesses.getNumber() + 1,
                pageProcesses.getTotalElements(), pageProcesses.getTotalPages(), size);
        model.addObject("processes", pageProcesses);
        model.addObject("navPage", navPage);
        model.setViewName("processes/list");
        return model;
    }
